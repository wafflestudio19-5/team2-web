...
function FollowPage() {
  ...
  useEffect(()=>{
    if(loc.pathname.slice('following' ? userContext.nowUserID.length+2 : 0, 'g'? userContext.nowUserID.length+11: 0)==='following'){
      setIsChosen('following');
    }
    else if(loc.pathname.slice('follower' ? userContext.nowUserID.length+2 : 0, 'g'? userContext.nowUserID.length+11: 0)==='followers'){
    ...
    <>
      <header className={styles.UserProfileHeader}>
        ...
        <div>
          ...
          <div className={styles.UserProfileHeaderID}>
            @user_id
          </div>
        </div>
      </header>
