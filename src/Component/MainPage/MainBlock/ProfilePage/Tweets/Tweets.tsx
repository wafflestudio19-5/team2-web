...
const Tweets = ({ loadNext, userData }: Props) => {
  ...
  const [tweetData, setTweetData] = useState<TweetData['TweetsType']>(userData.tweets);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const params = useParams();

  const getTweet = async () => {
    if(loadNextOkay){
    await axios.get(`/user/${params.id}/?page=${page}`)
      .then((response) => {
  ...
  return (
    <ul className={styles.tweetsItems}>
      {tweetData ? (
        tweetData.map(item => item.author ? <Tweet key={item.id} item={item} /> : <div></div>)
      ) : (
